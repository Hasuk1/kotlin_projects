...
1. [Базовые вопросы](#базовые-вопросы)
    ...
    1.9 [Что такое корень приложения?](#9-что-такое-корень-приложения)
...
3. [Структуры данных](#структуры-данных-шпаргалка-по-структурам-данных-в-java)
    ...
    3.3 [Что такое HashMap & TreeMap?](#3-что-такое-hashmap--treemap)

    3.4 [Что такое хеш-таблица?](#4-что-такое-хеш-таблица)
...
5. [Работа с сетью](#работа-с-сетью)
    ...
    5.4 [Что такое сериализация и десериализация?](#4-что-такое-сериализация-и-десериализация)
...
6. [Многопоточность](#многопоточность)
    ...
    6.8 [Что такое deadlock?](#8-что-такое-deadlock)

    6.9 [Какие варианты реализации многопоточности есть в Android?](#9-какие-варианты-реализации-многопоточности-есть-в-android)

    6.10 [Что такое main thread? Какие операции нужно выполнять на main thread, а какие нельзя делать?](#10-что-такое-main-thread-какие-операции-нужно-выполнять-на-main-thread-а-какие-нельзя-делать)

    6.11 [Что такое ключевое слово atomic и для чего оно нужно?](#11-что-такое-ключевое-слово-atomic-и-для-чего-оно-нужно)
...
9. [Android SDK](#android-sdk)
    ...
    9.2 [Что такое Activity?](#2-что-такое-activity)

    9.3 [Опишите жизненный цикл Activity.](#3-опишите-жизненный-цикл-activity)

    9.4 [Что такое Bundle saveInstanceState в методе onCreate?](#4-что-такое-bundle-saveinstancestate-в-методе-oncreate)

    9.5 [Какие типы Service знаете?](#5-какие-типы-service-знаете)

    9.6 [Что такое BroadcastReceiver и какие типы существуют?](#6-что-такое-broadcastreceiver-и-какие-типы-существуют)
    
    9.7 [Что такое ContentProvider?](#7-что-такое-contentprovider)
    
    9.8 [Что такое Фрагмент? Для чего используют механизм фрагментов?](#8-что-такое-фрагмент-для-чего-используют-механизм-фрагментов)
    
    9.9 [Опишите жизненный цикл Fragment.](#9-опишите-жизненный-цикл-fragment)
    
    9.10 [Есть ли у Fragment контекст? Если да, то как его получить?](#10-есть-ли-у-fragment-контекст-если-да-то-как-его-получить)
    
    9.11 [Чем отличается Fragment от Activity? Зачем выдумали Fragment?](#11-чем-отличается-fragment-от-activity-зачем-выдумали-fragment)
    
    9.12 [Что такое изменение конфигурации? Что происходит с приложением на Android при этом?](#12-что-такое-изменение-конфигурации-что-происходит-с-приложением-на-android-при-этом)
    
    9.13 [Что такое Intent? Что такое explicit/implicit Intent?](#13-что-такое-intent-что-такое-explicitimplicit-intent)
    
    9.14 [Что такое SharedPreferences?](#14-что-такое-sharedpreferences)
    
    9.15 [Что такое ANR? Как избегать таких ситуаций?](#15-что-такое-anr-как-избегать-таких-ситуаций)
    
    9.16 [Что такое DataBinding?](#16-что-такое-databinding)
    
    9.17 [Что такое LiveData? Какие виды знаете?](#17-что-такое-livedata-какие-виды-знаете)
    
    9.18 [Как создать ViewModel? Почему создавать ViewModel нужно именно так?](#18-как-создать-viewmodel-почему-создавать-viewmodel-нужно-именно-так)
    
    9.19 [Что такое Context и зачем он?](#19-что-такое-context-и-зачем-он)
    
    9.20 [Что такое AndroidManifest.xml? Зачем его используют? Что мы можем там декларировать?](#20-что-такое-androidmanifestxml-зачем-его-используют-что-мы-можем-там-декларировать)
    
    9.21 [Перечислите известные layout и их краткое описание.](#21-перечислите-известные-layout-и-их-краткое-описание)
    
    9.22 [Расскажите, что нужно реализовать, чтобы отобразить список строк в RecyclerView.](#22-расскажите-что-нужно-реализовать-чтобы-отобразить-список-строк-в-recyclerview)
    
    9.23 [Объясните паттерн ViewHolder. Для чего он применяется?](#23-объясните-паттерн-viewholder-для-чего-он-применяется)
    
    9.24 [Что такое DiffUtil?](#24-что-такое-diffutil)
    
    9.25 [Расскажите о ConstraintLayout.](#25-расскажите-о-constraintlayout)
    
    9.26 [Для чего используют Group, Guideline, Barriers, Chains в ConstraintLayout?](#26-для-чего-используют-group-guideline-barriers-chains-в-constraintlayout)
    
    9.27 [У вас есть Activity с двумя Fragment'ами, у одного есть кнопка, у другого TextView. При нажатии кнопки изменяется TextView. Как вы реализуете это?](#27-у-вас-есть-activity-с-двумя-fragmentами-у-одного-есть-кнопка-у-другого-textview-при-нажатии-кнопки-изменяется-textview-как-вы-реализуете-это)
    
    9.28 [Что такое WorkManager? Когда используем?](#28-что-такое-workmanager-когда-используем)
...
10. [Kotlin](#kotlin)
    ...
    10.18 [Что означает модификатор lateinit?](#18-что-означает-модификатор-lateinit--by-lazy)
    ...
    10.25 [Что такое Generics?](#25-что-такое-generics)
    
    10.26 [Что такое Wildcard?](#26-что-такое-wildcard)
    
    10.27 [Более подробно про ключевые слова in и out.](#27-более-подробно-про-ключевые-слова-in-и-out)
    
    10.28 [Что такое iner class и для чего он используется?](#28-что-такое-iner-class--nested-class-и-для-чего-он-используется)
    
    10.29 [Что такое data class?](#29-что-такое-data-class)

    10.30 [Что такое inline, crossinline и noinline, reified?](#30-что-такое-inline-crossinline-и-noinline-reified)
...
### **4. Что такое сериализация и десериализация?**
<details>
  <summary><b>Answer</b></summary>
    <UL>
...
<details>
  <summary><b>Как extensions выглядят в Java?</b></summary>
    <UL>
        <LI>Функции расширения для JVM являются static final методами.<br><br>Extensions в Kotlin не имеют прямого эквивалента в Java. Однако, компилятор Kotlin генерирует соответствующий статический метод в классе-компаньоне, который может быть вызван из Java кода в качестве статического метода этого класса.
...
### **18. Что означает модификатор lateinit & by lazy?**
<details>
  <summary><b>Answer</b></summary>
    <UL>
        <LI>Модификатор <b>lateinit</b> говорит о том, что данная переменная будет инициализирована позже. При этом инициализировать свойство можно из любого места, откуда она видна.</LI>
        <LI>Модификатор <b>by lazy</b> или <b>ленивая инициализация (lazy initialization)</b> — это подход, при котором объект инициализируется только при необходимости, а не сразу после создания. В Kotlin для ленивой инициализации свойств используется делегат lazy.</LI>
    </UL>
</details>
...
### **28. Что такое iner class & nested class и для чего он используется?**
<details>
  <summary><b>Answer</b></summary>
    <UL>
        <LI><b>Inner class (внутренний класс)</b> — класс, который определен внутри другого класса. Внутренний класс имеет доступ ко всем членам внешнего класса, включая закрытые и защищенные члены.<br><br>Это может быть удобно, когда вам нужен доступ к членам внешнего класса изнутри внутреннего класса.</LI><br>
        <b>Ключевое отличие:</b> внутренний (inner) класс — это вложенный (nested) класс, который может обращаться к компонентам внешнего класса.
    </UL>
</details>
...
### **30. Что такое inline, crossinline и noinline, reified?**
<details>
  <summary><b>Answer</b></summary>
    <UL>
        <LI><b>Модификатор inline</b> позволяет компилятору подставить тело функции прямо в место её вызова.</LI>
        <LI><b>Модификатор noinline.</b> Если же вы хотите, чтобы некоторые лямбды, переданные inline-функции, не были встроены, то отметьте их модификатором noinline. Встраиваемая лямбда может быть вызвана только внутри inline-функции, либо может быть передана в качестве встраиваемого аргумента. В то время как с noinline-функциями можно работать без ограничений: хранить внутри полей, передавать куда-либо и т.д.</LI><br>
        <LI><b>Модификатор crossinline</b> — используется для указания, что лямбда-выражение не может содержать нелокальных return, даже если оно передано в inline-функцию.</LI>
        <LI><b>reified</b> — это ключевое слово, которое может быть использовано только в inline-функциях. reified позволяет получить информацию о типе generic-параметра во время выполнения программы. В обычном случае, информация о типах стирается и недоступна во время выполнения, но с помощью reified можно сохранять эту информацию и использовать в других частях приложения.</LI>
    </UL>
</details>
