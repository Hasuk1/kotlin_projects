...
- ***Класс*** - это способ описания сущности. Его шаблон, который имеет минимальный набор общих характеристик для работы с ним. Классы являются ссылочнм типом данных, тоесть в куче(heap) будет храниться только ссылка на данные из стэка. 
